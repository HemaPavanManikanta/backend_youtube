...
const getResultFromCbse = async (req, res) => {
  try{
    ...
    // if number of requests are higher thann thhe set limit
    if(requests > limit){ // 1 > 5 = false
      return res.status(400).json({message : "Too many requests! try again in some time"})
    }

    return res.status(200).json({message : "Request fetched successful!", data : {result : "Passed"}})
